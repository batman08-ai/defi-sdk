...
contract YearnStakingV1Adapter is ProtocolAdapter {
        ...
        returns (uint256)
    {
        if (token == YFI) {
            ...
            totalRewards += IStakingRewards(CURVE).earned(account);
            totalRewards += IStakingRewards(BALANCER).earned(account);
            totalRewards += IStakingRewards(GOVERNANCE).earned(account);
            totalRewards += ERC20(FEE_REWARDS).balanceOf(account);
            return totalRewards;
        } else if (token == CURVE_Y) {
            uint256 totalRewards = 0;
            totalRewards += ERC20(CURVE).balanceOf(account);
            totalRewards += IStakingRewards(FEE_REWARDS).earned(account);
            return totalRewards;
