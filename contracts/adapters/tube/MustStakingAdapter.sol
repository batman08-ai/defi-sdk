...
contract MustStakingAdapter is ProtocolAdapter {
    ...
    address internal constant UNI_V2_WETH_POOL =
        0x15861b072abAd08b24460Add30b09E1481290F94;
    address internal constant STAKING_REWARD =
        0x048Dda990f581e80EFfc72E4e1996AE548f8d64C;
    ...
    {
        if (token == MUST) {
            return IStakingRewards(STAKING_REWARD).earned(account);
        } else if (token == UNI_V2_WETH_POOL) {
            return IStakingRewards(STAKING_REWARD).balanceOf(account);
        } else {
            return 0;
        }
    }
