...
contract BzxStakingAdapter is ProtocolAdapter {
        ...
        returns (uint256)
    {
        if (token == IBZRX || token == VBZRX || token == BPT) {
            return
                IStakingRewardsBzx(STAKING_CONTRACT).balanceOfByAsset(
                    token,
                    account
                );
        } else if (token == BZRX) {
            (uint256 bzrxEarnings, , , ) = IStakingRewardsBzx(STAKING_CONTRACT)
                .earned(account);
            return
                IStakingRewardsBzx(STAKING_CONTRACT).balanceOfByAsset(
                    token,
            ...
            (, uint256 curve3crv, , ) = IStakingRewardsBzx(STAKING_CONTRACT)
                .earned(account);
            return curve3crv;
        } else {
            return 0;
