...
contract PoolTogetherAdapter is ProtocolAdapter {
    ...
    function getBalance(address token, address account)
        ...
        returns (uint256)
    {
        uint256 totalBalance = IBasePool(token).totalBalanceOf(account);
        if (token == DAI_POOL) {
    ...
    function getPodBalance(address pod, address account)
        internal
        view
        returns (uint256)
    {
        return
            IPod(pod).balanceOfUnderlying(account) +
            IPod(pod).pendingDeposit(account);
    }
}
